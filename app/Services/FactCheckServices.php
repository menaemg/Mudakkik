    ...
    private function fetchUrlContent($url)
    {
        try {
            ...
            if ($response->successful()) {
                $body = $response->body();
                return (strlen($body) < 10) ? "FAILED_CONTENT" : mb_substr($body, 0, 2000);
            }
    ...
    private function finalVerdict($originalNews, $searchResults)
    {
        if (empty($searchResults)) {
            return [
                'rating' => 'غير مؤكد',
                'percentage' => 0,
                'summary' => 'لم نجد نتائج مطابقة في المصادر الموثوقة حالياً.',
                'evidence' => ''
            ];
        }
        ...
        try {
            ...
            $response = Http::withHeaders(['Authorization' => 'Bearer ' . env('GROQ_API_KEY')])
                ...
                ->post('https://api.groq.com/openai/v1/chat/completions', [
                    'model' => 'llama-3.1-8b-instant',
                    'messages' => [
                        ['role' => 'system', 'content' => 'أنت خبير تدقيق جنائي للأخبار والروابط. مهمتك الأولى هي تحديد ما إذا كانت نتائج البحث "مرتبطة سياقياً" بالادعاء أم لا. قواعد صارمة للحكم: 1. إذا كانت النتائج لا تتعلق بالموضوع: الرد {"rating": "غير مؤكد", "percentage": 0, ...}. 2. بناءً على نسبة ثقتك، اختر المسمى الصحيح: - أقل من 50%: "غير صحيح" - من 50% لـ 70%: "غير مؤكد" - من 70% لـ 90%: "صحيح" - أعلى من 90%: "رسمي" 3. اذكر الاقتباس الذي أكد أو نفى الخبر في حقل evidence. رد بـ JSON فقط بنفس هذا التنسيق.'],
            ...
            return [
                'rating' => 'غير مؤكد',
                'percentage' => 50,
                'summary' => "تعذر الربط التلقائي، والمصادر تشير إلى: " . ($searchResults[0]['title'] ?? 'أخبار متعلقة'),
                'evidence' => ''
            ];
        }
    }
