...
const MainEntertainCard = ({ post }) => (
    <Link href={route('posts.show', post.slug)} className="relative h-[600px] w-full
    group cursor-pointer overflow-hidden rounded-xl shadow-2xl block" data-aos="zoom-in">
        ...
        <div className="absolute bottom-8 right-8 left-8 bg-[#b20e1e]/95 backdrop-blur-sm p-8 text-white shadow-xl
        border-t-4 border-white/20 rounded-sm">
        ...
        <h2 className="text-3xl md:text-4xl font-black leading-tight mb-4 drop-shadow-md">{post.title}</h2>
            <div className="flex items-center gap-3 text-xs text-white/90 font-medium border-t border-white/20 pt-4">
                <Avatar className="w-8 h-8 border-2 border-white/30">
                    ...
                    <AvatarFallback className="bg-[#b20e1e] text-white font-bold">
...
const VerticalCard = ({ post, delay }) => (
    <Link href={route('posts.show', post.slug)} className="flex flex-col mb-8 last:mb-0 group cursor-pointer
    bg-white p-3 rounded-xl hover:shadow-xl transition-all duration-300 border border-transparent hover:border-gray-100 block" data-aos="fade-up" data-aos-delay={delay}>
...
const SideListItem = ({ post, delay }) => (
    ...
    className="mb-6 border-b border-gray-100 pb-4 last:border-0
     group hover:translate-x-[-5px] transition-transform duration-300 block text-right"
     data-aos="fade-left" data-aos-delay={delay}>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
            <Avatar className="w-5 h-5"><AvatarImage src={getImageUrl(post.user?.avatar)} /><AvatarFallback className="bg-[#b20e1e]
            text-white font-bold text-xs">{post.user?.name?.charAt(0)?.toUpperCase() || 'U'}</AvatarFallback></Avatar>
    ...
    return (
        <section className="container mx-auto px-4 py-16 bg-gray-50/30">
            <div className="mb-8 border-b border-gray-200 flex justify-between items-end pb-2">
                ...
                <Link href={route('posts.index', {category: 'entertainment'})} className="text-xs font-bold text-gray-500 hover:text-[#b20e1e]
                flex items-center gap-1 transition-colors mb-1 group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-3">
                    <div className="h-[200px] bg-gradient-to-br from-pink-600 to-rose-700 mb-6 flex flex-col items-center
