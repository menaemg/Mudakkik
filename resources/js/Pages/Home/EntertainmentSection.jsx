...
const MainCardSkeleton = () => (
    <div className="relative h-[600px] w-full rounded-xl bg-gray-200 animate-pulse overflow-hidden"></div>
);
const VerticalCardSkeleton = () => ( <div className="flex flex-col mb-8 last:mb-0 p-3"><div className="h-48 bg-gray-200 rounded-lg mb-3"></div></div> );
        ...
        <div className="absolute bottom-8 right-8 left-8 bg-[#b20e1e]/95 backdrop-blur-sm p-8 text-white shadow-xl border-t-4 border-white/20 rounded-sm">
            ...
            <div className="flex items-center gap-3 text-xs text-white/90 font-medium border-t border-white/20 pt-4">
                 <Avatar className="w-8 h-8 border-2 border-white/30">
                    ...
                    <AvatarFallback className="bg-[#b20e1e] text-white font-bold">{post.user?.name?.charAt(0)?.toUpperCase() || 'U'}</AvatarFallback>
                </Avatar>
...
const SideListItem = ({ post, delay }) => (
    <Link href={route('posts.show', post.slug)} className="mb-6 border-b border-gray-100 pb-4 last:border-0 group hover:translate-x-[-5px] transition-transform duration-300 block text-right" data-aos="fade-left" data-aos-delay={delay}>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
             <Avatar className="w-5 h-5"><AvatarImage src={getImageUrl(post.user?.avatar)} /><AvatarFallback className="bg-[#b20e1e] text-white font-bold text-xs">{post.user?.name?.charAt(0)?.toUpperCase() || 'U'}</AvatarFallback></Avatar>
    ...
    return (
        <div className="h-[200px] bg-gradient-to-br from-pink-600 to-rose-700 mb-6 flex flex-col items-center justify-center text-white text-center p-6 rounded-xl shadow-xl relative overflow-hidden group">
            {agendaAd && agendaAd.image && (
                 <img src={getImageUrl(agendaAd.image)} className="absolute inset-0 w-full h-full object-cover opacity-20 group-hover:opacity-30 transition-opacity duration-700" alt={title} />
            )}
            ...
            <div className="relative z-10 w-full">
                ...
                <a
                    ...
                    className="bg-white text-rose-600 px-5 py-2 text-[11px] font-bold rounded-full hover:scale-105 transition-transform inline-block shadow-sm hover:shadow-md cursor-pointer"
                >
                    {buttonText}
                </a>
            </div>
