    ...
    return (
        <div className="font-sans pb-20 px-4 md:px-8 rtl bg-[#F8FAFC] min-h-screen" dir="rtl">
            ...
            <div className="py-8 mb-8 border-b border-slate-200/60 flex justify-between items-center">
                ...
                <Button onClick={handleSubmit} disabled={processing} className="bg-brand-blue hover:bg-blue-800 text-white px-8 py-6 rounded-xl shadow-lg flex items-center gap-2 font-bold text-lg">
                ...
                <div className="xl:col-span-4 bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-xl mb-8">
                        <button
                            ...
                            className={`flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${data.type === 'post' ? 'bg-white text-brand-blue shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            <FaNewspaper /> عرض خبر
                        </button>
                        <button
                            onClick={() => setData('type', 'ad')}
                            className={`flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${data.type === 'ad' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            <FaAd /> عرض إعلان
                        </button>
                    </div>
                            ...
                            <div className="animate-in fade-in slide-in-from-left-4 duration-300 text-center space-y-4">
                                <div className="p-6 bg-amber-50 border border-amber-100 rounded-2xl">
                                    ...
                                    <Link
                                        ...
                                        className="inline-flex items-center gap-2 text-xs font-bold bg-white text-amber-700 border border-amber-200 px-4 py-2 rounded-lg hover:bg-amber-50 transition-colors"
                                    >
                                ...
                                {!activeAd && (
                                    <div className="flex items-center gap-2 text-red-500 bg-red-50 p-3 rounded-lg text-xs font-bold justify-center">
