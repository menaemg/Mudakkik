                        ...
                        <tr key={user.id} className="hover:bg-slate-50/50 transition-all group">
                            <td className="px-10 py-7">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="text-right">
                                        <div className="text-[17px] font-black text-[#001246] group-hover:text-[#D00000] transition-colors">{user.name}</div>
                                        <div className="text-[13px] text-slate-400 font-bold mt-0.5">{user.email}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td className="px-10 py-7 text-center">
                                {user.email_verified_at ? (
                                    ...
                                    <span className="text-[11px] font-black text-amber-700 bg-amber-100/50 px-3 py-1.5 rounded-full border border-amber-200">قيد الانتظار</span>
                                )}
                            </td>
                            <td className="px-10 py-7 text-center">
                                <div className="flex justify-center items-center gap-3">
                                    <button onClick={() => onView(user)} className="p-3 text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white rounded-[1rem] transition-all"><EyeIcon size={18} /></button>
                                    <button onClick={() => onEdit(user)} className="p-3 text-amber-600 bg-amber-50 hover:bg-amber-600 hover:text-white rounded-[1rem] transition-all"><Edit2 size={18} /></button>
                                    <button onClick={() => onDelete(user)} className="p-3 text-red-600 bg-red-50 hover:bg-red-600 hover:text-white rounded-[1rem] transition-all"><Trash2 size={18} /></button>
                                </div>
                            </td>
                        </tr>
                    ))}
