...
export default function PostView({ post, onBack }) {
    const handleImageError = (e) => {
        ...
        e.target.src =
            "https://images.unsplash.com/photo-1504711432869-efd5973bd14f?q=80&w=1000&auto=format&fit=crop";
    };

    const getAiVerdictStyle = (verdict) => {
        const styles = {
            trusted: {
                bg: "bg-green-50",
                text: "text-green-700",
        ...
        return styles[verdict] || {
            bg: "bg-slate-50",
            text: "text-slate-500",
            icon: <CheckCircle size={28} className="text-slate-400" />,
            label: "غير مراجع",
            border: "border-slate-100"
        };
    };

    const currentAi = getAiVerdictStyle(post.ai_verdict);

    return (
        <div
            className="bg-white rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden font-sans relative"
            dir="rtl"
        >
            <div className="p-6 border-b flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-50">
                ...
                <div className="flex gap-4">
                    ...
                    <div className="hidden md:flex bg-[#001246]/5 text-[#001246] px-5 py-2 rounded-2xl text-xs font-black items-center gap-2">
                        <ShieldCheck size={16} className="text-[#D00000]" />{" "}
                        لوحة التحكم
                    </div>
                </div>
            </div>
            ...
            <div className="p-8 md:p-12 max-w-6xl mx-auto text-right">
                <div className="mb-10">
                    <div className="flex gap-3 mb-6 justify-start">
                        ...
                        <span
                            className={`px-5 py-1.5 rounded-full text-xs font-black border ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 p-2">
                    ...
                    <div className={`flex items-center gap-5 p-6 rounded-[2.5rem] border group hover:shadow-xl transition-all duration-300 ${currentAi.bg} ${currentAi.border}`}>
                        ...
                        <div>
                            ...
                            <p className={`font-black text-lg ${currentAi.text}`}>
                                {currentAi.label}
                            </p>
                        </div>
                    </div>
