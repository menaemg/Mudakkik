...
export default function PostEdit({ post, categories, tags, onBack }) {
    ...
    return (
        <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden font-sans">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 text-[#001246] font-black hover:bg-slate-200 px-6 py-2 rounded-2xl transition-all border border-slate-200"
                >
            ...
            <form onSubmit={handleSubmit} className="p-8 md:p-12 max-w-5xl mx-auto space-y-10">
                <div className="space-y-4">
                    ...
                    <input
                        ...
                        className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] focus:border-[#D00000] focus:bg-white outline-none font-black text-2xl transition-all"
                        placeholder="اكتب العنوان هنا..."
                    />
                    {errors.title && (
                        <p className="text-red-500 text-sm font-bold mr-2">{errors.title}</p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        ...
                        <select
                            ...
                            className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none focus:border-[#D00000]"
                        >
                        ...
                        {errors.category_id && (
                            <p className="text-red-500 text-xs font-bold mr-2">{errors.category_id}</p>
                        )}
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        />
                        {errors.image && (
                            <p className="text-red-500 text-xs font-bold mr-2">{errors.image}</p>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-4 pt-6 border-t border-slate-50">
                    <label className="flex items-center gap-2 text-[#001246] font-black text-lg">
                        <Hash className="text-[#D00000]" size={20} /> الوسوم (التاجات)
                    </label>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100">
                        {tags?.map((tag) => (
                            <label
                                key={tag.id}
                                className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border-2 ${
                                    data.tag_ids.includes(tag.id)
                                        ? "bg-white border-[#D00000] shadow-sm"
                                        : "bg-transparent border-transparent hover:bg-white/50"
                                }`}
                            >
                                <input
                                    type="checkbox"
                                    className="w-5 h-5 rounded border-slate-300 text-[#D00000] focus:ring-[#D00000]"
                    ...
                    {errors.tag_ids && (
                        <p className="text-red-500 text-xs font-bold mr-2">{errors.tag_ids}</p>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    />
                    {errors.body && (
                        <p className="text-red-500 text-sm font-bold mr-2">{errors.body}</p>
                    )}
                </div>
