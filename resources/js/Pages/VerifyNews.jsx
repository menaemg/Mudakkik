...
export default function VerifyNews() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setResult({
        verdict: data.verdict?.label || data.verdict || "غير مؤكد",
        confidence_score:
          data.verdict?.confidence || data.confidence_score || 0,
        explantion:
          data.verdict?.summary ||
          data.ai_explantion ||
          "لم يتم العثور على تحليل مفصل.",
  ...
  return (
    <div
      className="min-h-screen bg-[#F8FAFF] font-sans text-right pb-10"
      dir="rtl"
    >
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-8">
        <header className="flex items-center justify-between mb-10 bg-white p-4 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black text-[#001246]">
              كاشف التضليل الرقمي
            </h1>
          </div>
          ...
          >
            <div className="bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/5 p-2 border border-slate-100">
              ...
              <div className="p-4 border-t border-slate-50 flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 mr-4">
                  {news.length} حرف
                </span>
                <button
            ...
            <AnimatePresence mode="wait">
              {!result && !loading && (
                ...
                >
                  ...
                  <p className="font-bold text-xl">
                    نتائج الفحص ستظهر هنا فور البدء
                  </p>
                </motion.div>
              )}
              ...
              {loading && (
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-black text-[#001246] text-xl mb-2">
                      جاري فحص المصادر الرقمية
                    </p>
                    <p className="text-slate-400 text-sm animate-pulse">
                      يتم الآن التحقق من قواعد البيانات والمصادر الموثوقة...
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              {result && !loading && (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  className={`rounded-[3rem] shadow-2xl overflow-hidden border-t-[12px] ${
                    result.verdict.includes("صحيح")
                      ? "bg-white border-emerald-500"
                      : "bg-white border-[#D00000]"
                  }`}
                >
                  <div className="p-8 md:p-12">
                    <div className="flex items-center justify-between mb-10 bg-[#F8FAFC] p-6 rounded-[2rem]">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <span className="text-xs font-black text-slate-400 block mb-1 uppercase tracking-tighter">
                            حكم النظام النهائي
                          </span>
                          <h3
                      ...
                      <div className="text-left">
                        ...
                        <span className="text-4xl font-black text-[#001246]">
                          {String(result.confidence_score).replace("%", "")}%
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="font-black text-[#001246] mb-4 px-2">
                        المصادر المكتشفة
                      </h4>
                      <div className="space-y-3">
                        {result.sources?.length > 0 ? (
                          result.sources.map((source, index) => (
                            ...
                            >
                              <span className="font-bold text-[#001246] text-sm truncate max-w-[80%]">
                                {source.title}
                              </span>
                              <ExternalLink
                                size={16}
                                className="text-slate-300 group-hover:text-[#D00000]"
                              />
                            </motion.a>
                          ))
                        ) : (
                          <p className="text-slate-400 text-sm px-2 italic">
                            لم يتم العثور على روابط مباشرة.
                          </p>
                        )}
                      </div>
                    </div>
