...
import { Search, CheckCircle, XCircle, AlertCircle, Loader2, ExternalLink, ShieldCheck, RefreshCcw, History } from 'lucide-react';
                            ...
                            {loading && (
                                ...
                                >
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-slate-400 text-sm animate-pulse">يتم الآن التحقق من أكثر من 10 مصادر موثوقة...</p>
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {result && !loading && (
                                <motion.div 
                                    ...
                                    className={`rounded-[3rem] shadow-2xl overflow-hidden border-t-[12px] ${
                                        result.verdict === 'صحيح' ? 'bg-white border-emerald-500' : 'bg-white border-[#D00000]'
                                    }`}
                                >
                                    <div className="p-8 md:p-12">
                                        ...
                                        <div className="flex items-center justify-between mb-10 bg-[#F8FAFC] p-6 rounded-[2rem]">
                                            <div className="flex items-center gap-4">
                                                <div className={`p-4 rounded-2xl ${result.verdict === 'صحيح' ? 'bg-emerald-500' : 'bg-[#D00000]'} text-white shadow-lg`}>
                                                    {result.verdict === 'صحيح' ? <CheckCircle size={32} /> : <XCircle size={32} />}
                                                </div>
                                                <div>
                                                    <span className="text-xs font-black text-slate-400 block mb-1 uppercase tracking-tighter">حكم النظام النهائي</span>
                                                    <h3 className={`text-4xl font-black ${result.verdict === 'صحيح' ? 'text-emerald-600' : 'text-[#D00000]'}`}>{result.verdict}</h3>
                                                </div>
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-3">
                                                {result.sources?.map((source, index) => (
                                                    <motion.a 
                                                        whileHover={{ x: -10 }}
                                                        key={index} href={source.url} target="_blank"
