...
import { 
    ...
    Menu, X, Bell, ChevronLeft, Search, ShieldCheck 
} from "lucide-react";
...
<div className="flex h-screen bg-gradient-to-tr from-[#F1F5F9] via-[#F8FAFC] to-[#E2E8F0] font-sans text-right overflow-hidden" dir="rtl">            
            ...
            `}>
                ...
                <div className="p-8 text-2xl font-black flex items-center gap-3 bg-black/20 backdrop-blur-xl border-b border-white/5">
                    ...
                    <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden text-white mr-auto hover:bg-white/10 p-1 rounded">
                        <X size={24} />
                    </button>
                </div>
                ...
                <nav className="p-6 space-y-3 mt-4 overflow-y-auto">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[3px] mb-4 mr-2">القائمة الرئيسية</p>
                    {menuItems.map((item) => (
                        <Link 
                            key={item.url} 
                            href={item.url} 
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <item.icon size={20} className="group-hover:text-[#FF4D4D] transition-colors" />
                                <span className="font-bold text-sm tracking-wide">{item.label}</span>
                            </div>
                            <ChevronLeft size={14} className="opacity-0 group-hover:opacity-100 transition-all translate-x-1 group-hover:translate-x-0" />
                        </Link>
                    ))}
                </nav>
                ...
                <header className="bg-white/70 backdrop-blur-md h-20 border-b border-slate-200/60 flex items-center justify-between px-8 z-40">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden md:flex items-center bg-slate-100/80 rounded-2xl px-4 py-2.5 gap-3 border border-slate-200/50 focus-within:border-[#001246] focus-within:bg-white focus-within:shadow-xl focus-within:shadow-blue-900/5 transition-all w-80 group">
                            <Search size={18} className="text-slate-400 group-focus-within:text-[#001246]" />
                            <input type="text" placeholder="ابحث في نظام المـدقق..." className="bg-transparent border-none text-sm outline-none w-full text-right font-medium" />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex items-center gap-3 pr-4 border-r border-slate-200">
                            <div className="text-left hidden sm:block">
                                <p className="text-xs font-black text-[#001246]">آية أحمد</p>
                                <p className="text-[10px] text-slate-400 font-bold tracking-tighter uppercase">Admin</p>
                            </div>
